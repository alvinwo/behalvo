...
import { createUsVisaChinaPolicyAdapter } from './policy.js';
...
import {
  ...
  type UsVisaChinaBooking, type UsVisaChinaExecutionInput, type UsVisaChinaExecutionResult,
...
export function createSyntheticUsVisaChinaExecutionAdapter(input: { session: BrowserSession }): MonitoredActionPolicyAdapter {
  ...
  const portal = (fence: TrustedExecutionFence, intentId?: string): UsVisaChinaSyntheticPortalPort => ({
    ...
    recordDurableIntent: async (recorded, slotId) => {
      ...
      await input.session.gesture({ kind: 'booking.intent', slotId, intentId: recorded }, 'calendar', fence);
    },
    gesture: async (value: unknown) => {
      ...
      const snapshot = await input.session.gesture(command as never, expectedState, fence);
  ...
  return { ...policy,
    async executeReserved({ action, grant, connection, fence, intentId, recordConfirmation }) {
      ...
      const result = await executeSyntheticUsVisaBooking({ action, portal: portal(fence, intentId),
        ...
        preflight: () => ({ appointmentAbsent: true, identityDigest: args.identityDigest,
          rosterDigest: args.candidate.rosterDigest, termsDigest: args.termsDigest, candidate: args.candidate }),
